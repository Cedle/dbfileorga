...
public class MitgliederDB implements Iterable<Record> {
    ...
    public Record read(int recNum) {
        ...
        if (blockNum != -1) {
            ...
            for (int i = 0; i < blockNum; i++) {
                count = count - getBlock(i).getNumberOfRecords();
            }
            DBBlock block = getBlock(blockNum);

            return block.getRecord(count);
        } else {
            return null;
        }
    }
    ...
    public int findPos(String searchTerm) {
        for (int i = 1; i <= this.getNumberOfRecords(); i++) {
            if (read(i).getAttribute(1).equals(searchTerm)) {
                return i;
            }
        }
        return -1;
    ...
    public int insert(Record record) {
        int ret = this.appendRecord(record);
        if (ret != -1) {
    ...
    private int getNumberOfFirstRecordFormBlock(int blockNum) {
